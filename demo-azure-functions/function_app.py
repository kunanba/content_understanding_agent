...
import json
import logging
# Updated: 2026-01-12 - Fixed JSON serialization bugs
...
def perform_ocr(req: func.HttpRequest) -> func.HttpResponse:
    ...
    Expected parameters:
    - classifier_id OR analyzer_id: The classifier/analyzer ID to use for document processing
    ...
    try:
        ...
        classifier_id = req.params.get('classifier_id') or req.params.get('analyzer_id')
        ...
        if not all([classifier_id, blob_url, storage_account_name]):
            try:
                ...
                if req_body:
                    classifier_id = classifier_id or req_body.get('classifier_id') or req_body.get('analyzer_id')
        ...
        if not all([classifier_id, blob_url, storage_account_name]):
            return func.HttpResponse(
                json.dumps({
                    ...
                    "message": "classifier_id (or analyzer_id), blob_url, and storage_account_name are required"
                }),
                status_code=400,
...
def perform_ocr_processing(classifier_id: str, blob_url: str, storage_account_name: str) -> dict:
    ...
    # Optional: Support both managed identity and client secret
    ...
    if not all([endpoint, api_version]):
        raise ValueError("Missing required environment variables: SERVICE_FOR_CU, SERVICE_API_FOR_CU")

    # Create the credential - prefer managed identity, fallback to client secret
    if client_id and client_secret and tenant_id:
        logging.info("Using ClientSecretCredential")
    ...
    # Generate SAS URL for the blob so Content Understanding service can access it
    from azure.storage.blob import generate_blob_sas, BlobSasPermissions
    from datetime import timedelta
    
    # Parse blob URL to get container and blob name
    blob_parts = blob_url.replace(f"https://{storage_account_name}.blob.core.windows.net/", "").split("/", 1)
    if len(blob_parts) == 2:
        container_name_source = blob_parts[0]
        blob_name = blob_parts[1]
        ...
        # Update blob_url with SAS token
        blob_url_with_sas = f"{blob_url}?{sas_token}"
        logging.info(f"✅ Generated SAS token for blob access")
    else:
        blob_url_with_sas = blob_url
        logging.warning("⚠️ Could not parse blob URL, using original URL")
    ...
    # Process with analyzer (or classifier)
    ocr_result_json = None
    if classifier_id:
        logging.info(f"🔨 Using analyzer/classifier: {classifier_id}")
        logging.info("📄 Processing document")
        logging.info(f"   Document: {blob_url_with_sas[:100]}...")
        
        # Try analyzer API first
        try:
            logging.info("⏳ Trying analyzer API...")
            response = content_understanding_client.begin_analyze(analyzer_id=classifier_id, file_location=blob_url_with_sas)
            logging.info("✅ Using analyzer API")
        except Exception as analyzer_error:
            # Fall back to classifier API
            logging.info(f"ℹ️ Analyzer failed: {str(analyzer_error)[:100]}")
            logging.info("⏳ Trying classifier API...")
            try:
                response = content_understanding_client.begin_classify(classifier_id=classifier_id, file_location=blob_url_with_sas)
                logging.info("✅ Using classifier API")
            except Exception as classifier_error:
                logging.error(f"❌ Both analyzer and classifier APIs failed")
                logging.error(f"Analyzer error: {analyzer_error}")
                logging.error(f"Classifier error: {classifier_error}")
                raise ValueError(f"Failed to process document. Analyzer error: {analyzer_error}. Classifier error: {classifier_error}")
        ...
        try:
            ...
            logging.error(f"❌ Error polling result: {e}")
            raise
    else:
        raise ValueError("⚠️ analyzer/classifier ID not provided.")

    # Upload results to blob storage using the same credential
    storage_account_url = f"https://{storage_account_name}.blob.core.windows.net"
    blob_service_client = BlobServiceClient(account_url=storage_account_url, credential=credential)
    ...
    ocr_result_blob_name = f"{original_file_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def create_excel(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        # Use service principal for storage access
...
def parse_ocr(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        summary_result = produce_summary_report(ocr_result_json, storage_account_name, ocr_result_blob_name, credential)
        ...
        return func.HttpResponse(
            json.dumps({
                ...
                "summary_report_blob_name": summary_result.get("summary_report_blob_name"),
                "summary_container_name": summary_result.get("summary_container_name")
            }),
            status_code=200,
            mimetype="application/json"
        )
